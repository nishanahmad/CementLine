...
if(isset($_SESSION["user_name"]))
{
	...
	$arObjects =  mysqli_query($con,"SELECT id,name,mobile,shop_name,sap_code FROM clients WHERE isActive = 1") or die(mysqli_error($con));		 
	...
	{	
		...
		$sales = mysqli_query($con,"SELECT ar_id,SUM(srp),SUM(srh),SUM(f2r),SUM(return_bag) FROM sales WHERE '$year' = year(`entry_date`) AND '$month' = month(`entry_date`) AND ar_id IN ('$arIds') GROUP BY ar_id") or die(mysqli_error($con));	
	...
	else
	{
		...
		foreach($specialTargetObjects as $specialTarget)
		{
			...
			$sales = mysqli_query($con,"SELECT ar_id,SUM(srp),SUM(srh),SUM(f2r),SUM(return_bag) FROM sales WHERE entry_date >= '$start' AND entry_date <= '$end' AND ar_id = '$arId' GROUP BY ar_id") or die(mysqli_error($con));	
			foreach($sales as $sale)
			{
				$total = $sale['SUM(srp)'] + $sale['SUM(srh)'] + $sale['SUM(f2r)'] - $sale['SUM(return_bag)'];
...
function getPrevPoints($arList,$endYear,$endMonth,$dateString)
{
	...
	foreach($stDates as $stDate)
	{
		...
		$end = $stDate['to_date'];
		$sales = mysqli_query($con,"SELECT ar_id,SUM(srp),SUM(srh),SUM(f2r),SUM(return_bag) FROM sales WHERE entry_date >= '$start' AND entry_date <= '$end' AND ar_id IN ('$arIds') GROUP BY ar_id") or die(mysqli_error($con));	
		foreach($sales as $sale)
		{
			$arId = $sale['ar_id'];
			$total = $sale['SUM(srp)'] + $sale['SUM(srh)'] + $sale['SUM(f2r)'] - $sale['SUM(return_bag)'];
